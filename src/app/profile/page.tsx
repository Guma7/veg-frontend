              ...
              {recipes.map((recipe) => (
                <RecipeCard key={recipe.id}>
                  <Link href={`/receitas/${recipe.slug || recipe.id}`} style={{ textDecoration: 'none' }}>
                    <RecipeImage 
                      src={recipe.image_url || (recipe.image ? (recipe.image.startsWith('http') ? recipe.image : recipe.image.startsWith('/') ? `${API_URL}${recipe.image}` : `${API_URL}/${recipe.image}`) : '/default-recipe.jpg')}
                      alt={recipe.title} 
                      onError={(e) => {
